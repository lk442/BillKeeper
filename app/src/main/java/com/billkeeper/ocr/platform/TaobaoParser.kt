 ...
 * 淘宝支付解析模板
 */
class TaobaoParser : PlatformParser {
    ...
    override fun parse(ocrResult: OcrResult): ParsedBillInfo {
        ...
        val isTaobaoPage = text.contains("淘宝")
        if (!isTaobaoPage) return ParsedBillInfo(rawText = text)
        ...
        var score = 0.3f
        if (text.contains("淘宝")) score += 0.2f
        ...
        return ParsedBillInfo(
            ...
            payee = payee,
            payerAccount = "淘宝支付",
            platform = PaymentPlatform.TAOBAO,

 ...
 * 美团支付解析模板
 */
class MeituanParser : PlatformParser {
    ...
    override fun parse(ocrResult: OcrResult): ParsedBillInfo {
        ...
        val isMeituanPage = text.contains("美团")
        if (!isMeituanPage) return ParsedBillInfo(rawText = text)
        ...
        var score = 0.3f
        if (text.contains("美团")) score += 0.2f
        ...
        return ParsedBillInfo(
            ...
            payee = payee,
            payerAccount = "美团支付",
            platform = PaymentPlatform.MEITUAN,
